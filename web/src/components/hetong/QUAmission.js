...
export default class Component_Hetong_QUAmission extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      /*表格的定义*/
  ...
  getQUAmissionTable = () => {
    ...
    var path = "type=hetong&fun=QUAmission";
    if (config.hasRule("302")) {
    } else {
      path = path + "&userID=" + userID;
    }
    var result = AJAX.send(path);
    result
      ...
      .then(function (json) {
        ...
        if (data.type == 0) {
          ...
          message.success(
            "获取到完成的mission：" + data.data.length + "条数据"
          );
    ...
    result
      ...
      .then(function (json) {
        ...
        if (data.type == 0) {
          ...
          _this.getTasksById(_this.state.id);
          _this.setState({
            accountModalVisible: false,
            accountModalTaskId: "",
            accountModalTaskDate: "",
    ...
    return (
      <div>
        ...
        <div style={{ padding: 24, background: "#fff", minHeight: 360 }}>
          <Component_TabSwitch activeId={"4"}></Component_TabSwitch>
          <div>
            <Row>
              <Button
                type="default"
            ...
            <Table
              title={() => "正在质保中的QUA-mission"}
              footer={() => "合计总共" + this.state.data.length + "条信息"}
              size={"small"}
  ...
  tableToCsv = () => {
    console.info("table to csv 暂时没有做");
  };
