...
export default class Component_Hetong_Mission extends React.Component{
  ...
  componentDidMount(){
    this.getFinishedMissionTable();
  }

//
getFinishedMissionTable=()=>{
  var _this=this;
  let userID=COOKIE.get('id');
  var path="type=hetong&fun=finishedMission"
  ...
  //  console.log(response)
    return response.json()
      ...
      message.success('获取到完成的mission：'+data.data.length+'条数据');
...
console.info("type=hetong&fun=MissionChange&data="+JSON.stringify(data ));
          ...
          <Collapse bordered={false} >
              <Panel style={{marginLeft:'50px',fontSize:'12px',color:'#f00',border:'0px'}} header="时间调整框" key="1" >
                  <Row className='row_bottom0' gutter={16}>
                   ...
                   <Col span={5}>
                      <InputNumber min={0} value={this.state.accountModalTaskDelayDays}
  ...
  render(){
    ...
    />
      ...
      <Component_TabSwitch activeId={'2'}></Component_TabSwitch>
      ...
      <div>
        ...
        <Table  title={() => '已完结的Mission'}   footer={() => '合计总共'+this.state.data.length+'条信息'}size={'small'} pagination={{showSizeChanger:true}} columns={this.state.columns} dataSource={this.state.data} />
  ...
  getAddMissionModalValue=(e)=>{
    ...
    //  console.log(response)
      return response.json()
        ...
        message.success('获取到task记录'+data.data.length+'条数据');
        console.info(data.data);
        _this.setState({
          data:data.data,
        });
