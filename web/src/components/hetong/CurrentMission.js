...
export default class Component_Hetong_CurrentMission extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      /*表格的定义*/
      columns: [
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              {record.object.length > 15
                ? record.object.substring(0, 15) + "..."
                : record.object}
            </span>
          ),
          key: "object",
        },
        {
          ...
          render: (text, record) => (
            <span>
              {record.factory.length > 8
                ? record.factory.substring(2, 8) + "..."
                : record.factory}
            </span>
          ),
        },
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              {record.c_time}
              {STR.minersNow(record.c_time) > 0 ? (
                <Badge count={STR.minersNow(record.c_time)} />
              ) : (
                <Badge
                  count={STR.minersNow(record.c_time)}
        ...
        },
        {
          ...
          render: (text, record) => (
            <span>
              {record.e_time}
              {STR.minersNow(record.e_time) > 0 ? (
                <Badge count={STR.minersNow(record.e_time)} />
              ) : (
                <Badge
                  count={STR.minersNow(record.e_time)}
                  style={{ backgroundColor: "#52c41a" }}
                />
              )}
            </span>
          ...
          render: (text, record) => (
            <span>
              ...
              <a
                ...
                onClick={this.rightSider.bind(this, record)}
              >
  ...
  //获取当前mission表
  getCurrentMissionTable = () => {
    var _this = this;
    let id = COOKIE.get("id");
    //console.info('cookies id:',id)
    let link = "type=hetong&fun=currentMission";
    if (config.hasRule("302")) {
      console.info("本账户号有302功能");
    } else {
      console.info("本账户没有302功能");
  ...
  rightSider = (record, event) => {
    console.info("rightsider open" + record.id);
    //将行数据载入到state

    //触发borderFrame bf 的打开方法
    //  console.info(this.state.id);
    this.setState({
      currentMissionId: record.id + "",
    });
    this.refs.bf.open();
    console.info("getTasksbyMissionid", record.id);
    this.getTasksByMissionId(record.id);
  };
  //获取数据刷新history
  getTasksByMissionId(id) {
    var _this = this;
    //  console.info('gethistory:'+id);
    var result = AJAX.send(
      "type=hetong&fun=MissionInfobyMissionId&MissionId=" + id
    );
    result
      .then(function (response) {
        console.log(response);
  ...
  addRightInfom = () => {
    ...
    var result = AJAX.send(
      "type=wuye&fun=addInform&data=" + JSON.stringify(data)
    );
    result
      .then(function (response) {
        //  console.log(response)
      ...
      .then(function (json) {
        ...
        if (data.type == 0) {
          ...
          _this.getTasksByMissionId();
          _this.getCurrentMissionTable();
        }
      })
      .catch(function (ex) {
        console.log("parsing failed", ex);
  ...
  renderTimeline() {
    ...
    var remeanPrices = (
      parseFloat(this.state.prices) - parseFloat(pay_sum)
    ).toFixed(2);
    return (
      <Timeline>
        {
                ...
                <p className="Timeline_right_2">
                  <Row>
                    <Col span={20}>
                      ...
                      />
                      {_this.state.currentTaskId == i.id ? (
                        <Badge
                          count="本期"
                          style={{ backgroundColor: "#52c41a" }}
                        />
                    ...
                    <Col span={4}>
                      {i.isfinished == 1 ? (
                        ...
                        <Button
                          size="small"
                          disabled={config.hasRule(304) ? false : true}
                          onClick={_this.account.bind(
                            this,
                            i.id,
                            i.date,
                            i.pay,
                    ...
                    {index + 1 == _this.state.tasks.length ? (
                      <Badge
                        count="最后一期!"
                        style={{ backgroundColor: "#cf1322" }}
                      />
                  ...
                  <Row gutter={24}>
                    ...
                    >
                      <Row>
                        <Col span={10} className="infoWord">
                          此期金额:
                        </Col>
                        <Col span={14} className="pricesNum">
                          {parseFloat(i.prices).toFixed(2)}
                        </Col>
                      </Row>
                      <Row>
                        <Col span={10} className="infoWord">
                      ...
                      <Row>
                        ...
                        >
                          {parseFloat(i.prices - i.pay).toFixed(2)}
                        </Col>
                      </Row>
                    </Col>
                    <Col span={16}>
                      <Row>
                        <span className="infoWord .gray">本期需做：</span>
                        {i.wtd}
                      </Row>
                      <Row>
                        <span className="infoWord .gray">本期做了：</span>
                        {i.dw}
                      </Row>
                    </Col>
                  </Row>
        ...
        <Timeline.Item>
          ...
          <p className="Timeline_right_2">
            <Row gutter={24}>
              <Col
                span={12}
                style={{ paddingRight: "5px", borderRight: "1px #ddd solid" }}
              >
                <Row>
                  <Col span={10} className="infoWord">
                    任务总金额：
                  ...
                  <Col span={14} className="pricesNum">
                    {parseFloat(this.state.prices).toFixed(2)}
                  </Col>
                </Row>
                <Row>
                  <Col span={10} className="infoWord">
              ...
              <Col span={10} style={{ fontSize: "50px" }}>
                {parseInt(
                  (parseFloat(pay_sum) / parseFloat(this.state.prices)) * 100
                )}
                %
              </Col>
            </Row>
        ...
        <Timeline.Item>
          ...
          >
            ...
            <Button
              type="primary"
              size="normal"
              onClick={this.eval_modal_open.bind(this)}
            >
              更改评价
            </Button>
          </p>
  ...
  accountModalOk = () => {
    ...
    if (this.state.account_MissiontoQUA) {
      ...
      console.info(
        "type=hetong&fun=MissionChange&data=" + JSON.stringify(data)
      );
    }
    var data = {
    ...
    var result = AJAX.send(
      "type=hetong&fun=TaskChange&data=" + JSON.stringify(data)
    );
    var _this = this;
    result
      .then(function (response) {
      ...
      .then(function (json) {
        ...
        if (data.type == 0) {
          ...
          _this.getTasksByMissionId(_this.state.id);
          _this.getCurrentMissionTable();
          _this.setState({
            accountModalVisible: false,
            accountModalTaskId: "",
    ...
    return (
      ...
      >
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={14}>
            <Badge
              count={"ID:" + this.state.accountModalTaskId}
              style={{
                backgroundColor: "#fff",
                color: "#999",
                boxShadow: "0 0 0 1px #d9d9d9 inset",
        ...
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={8}>
            <Input
              disabled={true}
              size="small"
              value={this.state.accountModalTaskDate}
            />
          </Col>
        </Row>
          ...
          >
            <Row className="row_bottom0" gutter={16}>
              ...
              <Col span={5}>
                <InputNumber
                  min={-5}
                  value={this.state.accountModalTaskDelayDays}
                  onChange={(e) => {
                    var d = e;
                    if (e == "") {
                      d = 0;
                    }
                    this.setState({ accountModalTaskDelayDays: d });
                  }}
                />
              </Col>
              <Col style={{ marginLeft: "20px" }} span={10}>
                <Switch
                  onChange={(e) => {
                    this.setState({ accountModalTaskDelayAllTask: e });
                  }}
                  defaultChecked={false}
                  checkedChildren="延后之后的Task"
                  unCheckedChildren="只延后本期"
                />
              </Col>
            </Row>
          </Panel>
        ...
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={14}>
            <Input
              disabled={true}
              size="small"
              value={this.state.accountModalTaskPrices}
            />
          </Col>
        </Row>
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={14}>
            <Input
              disabled={true}
              size="small"
              value={this.state.accountModalTaskPay}
            />
          </Col>
        </Row>
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={7}>
            <InputNumber
              min={0}
              max={
                this.state.accountModalTaskPrices -
                this.state.accountModalTaskPay
              }
              value={this.state.accountModalCurrentPay}
              defaultValue={0}
              onChange={(ev) => {
                var d = ev;
                if (ev == "") {
                ...
                this.setState({ accountModalCurrentPay: d });
              }}
            />
          </Col>
          <Col span={7}>
            <Switch
              onClick={(e) => {
                this.setState({ accountModalIsfinished: e });
              }}
              defaultChecked={false}
              checkedChildren="完结本期"
        ...
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={14}>
            <Input
              disabled={true}
              size="small"
              value={this.state.accountModalTaskWtd}
            />
          </Col>
        </Row>
        <Row>
          ...
          <Col span={14}>
            <Switch
              onClick={(e) => {
                this.setState({ account_MissiontoQUA: e });
              }}
              defaultChecked={false}
              checkedChildren="转入质保"
              unCheckedChildren="不变"
            />
          </Col>
        </Row>
        <Row className="row_bottom10" gutter={16}>
          ...
          <Col span={14}>
            <TextArea
              rows={4}
              onChange={(ev) =>
                this.setState({ accountModalTaskDw: ev.target.value })
              }
              value={this.state.accountModalTaskDw}
            ></TextArea>
          </Col>
        </Row>
      </Modal>
    ...
    return (
      <div>
        ...
        <div style={{ padding: 24, background: "#fff", minHeight: 360 }}>
          <Component_TabSwitch activeId={"3"}></Component_TabSwitch>

          <div>
            <Row>
              <Button
                type="default"
                disabled={config.hasRule(303) ? false : true}
                onClick={this.showAddMissionModal}
              ...
              <div style={{ fontSize: "16px" }}>
                ...
                <Row>
                  ...
                  <Col span={8} className="rightBlock">
                    ...
                    <Row>
                      <span className="rightTxt18" style={{ fontSize: "8px" }}>
                        {this.state.comp}
                      </span>
                    </Row>
                  </Col>
                ...
                <Row>
                  ...
                  <Col span={4} className="rightBlock">
                    ...
                    <Row>
                      <span className="rightTxt16" style={{ fontSize: "8px" }}>
                        {this.state.tel}
                      </span>
                    </Row>
                  </Col>
